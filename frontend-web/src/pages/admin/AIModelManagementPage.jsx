...
import UploadTrainingDataModal from '../../components/admin/UploadTrainingDataModal';
...
const AIModelManagementPage = () => {
  ...
  // Handle upload training data
  const handleUploadTrainingData = async (file, modelName) => {
    setUploading(true);
    try {
      const result = await aiService.uploadTrainingData(file, modelName);
      if (result.success) {
      ...
      // If API not available, show mock success
      console.error('Upload error:', error);
      message.success('Upload training data thành công! (Mock)');
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#F9FAFB' }}>
      ...
      <UploadTrainingDataModal
        open={showUploadModal}
        onClose={() => setShowUploadModal(false)}
        onUpload={handleUploadTrainingData}
        models={models}
      />
    </div>
  );
