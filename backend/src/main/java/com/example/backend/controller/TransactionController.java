...
public class TransactionController {
        ...
        @Operation(summary = "Tìm kiếm giao dịch theo từ khóa", description = """
                        Tìm kiếm giao dịch theo từ khóa trong mô tả, ghi chú và tên danh mục.

                        **Ví dụ:**
                        - Tìm giao dịch liên quan đến cafe: `/api/transactions/search?keyword=cafe`
                        - Tìm với phân trang: `/api/transactions/search?keyword=uber&page=0&size=10`
                        """)
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Thành công"),
                        @ApiResponse(responseCode = "401", description = "Chưa đăng nhập")
        ...
        public ResponseEntity<PageResponse<TransactionResponse>> searchTransactions(
                        @Parameter(description = "Từ khóa tìm kiếm (tìm trong mô tả, ghi chú, tên danh mục)", required = true) @RequestParam String keyword,
                        @Parameter(description = "Loại giao dịch: INCOME hoặc EXPENSE") @RequestParam(required = false) String type,
                        @Parameter(description = "Từ ngày (yyyy-MM-dd)") @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate startDate,
                        @Parameter(description = "Đến ngày (yyyy-MM-dd)") @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate endDate,
                        @Parameter(description = "Số trang (bắt đầu từ 0)") @RequestParam(defaultValue = "0") int page,
                        @Parameter(description = "Số items mỗi trang") @RequestParam(defaultValue = "20") int size,
                ...
                // Build filter object với keyword
                TransactionFilter filter = new TransactionFilter();
                filter.setKeyword(keyword);
                filter.setType(type);
                filter.setStartDate(startDate != null ? startDate.atStartOfDay() : null);
