...
public class StatisticsService {
    ...
    /**
     * Get income vs expense comparison for a period
     */
    ...
    public IncomeExpenseComparisonDTO getIncomeExpenseComparison(String username, LocalDate startDate,
        ...
        Long incomeCount = transactionRepository
                .countByUserIdAndTypeAndDateRange(user.getId(), Transaction.TransactionType.INCOME, start, end);
        Long expenseCount = transactionRepository
                .countByUserIdAndTypeAndDateRange(user.getId(), Transaction.TransactionType.EXPENSE, start, end);
        ...
        // Calculate averages
        if (incomeCount > 0) {
            dto.setAverageIncome(dto.getTotalIncome().divide(BigDecimal.valueOf(incomeCount), 2, RoundingMode.HALF_UP));
        } else {
        ...
        if (expenseCount > 0) {
            dto.setAverageExpense(
                    dto.getTotalExpense().divide(BigDecimal.valueOf(expenseCount), 2, RoundingMode.HALF_UP));
        } else {
    ...
    public List<SpendingInsightDTO> getSpendingInsights(String username) {
        ...
        merchantFrequency.entrySet().stream()
                ...
                .forEach(entry -> {
                    ...
                    insight.setDescription(
                            "You've spent at " + entry.getKey() + " " + entry.getValue() + " times this month");
        ...
        categorySpending.entrySet().stream()
                ...
                .forEach(entry -> {
                    ...
                    insight.setDescription(entry.getKey() + " accounts for "
                            + percentage.setScale(1, RoundingMode.HALF_UP) + "% of your spending");
        ...
        if (expenseCount > 0) {
            BigDecimal averageAmount = avgExpense.divide(BigDecimal.valueOf(expenseCount), 2, RoundingMode.HALF_UP);
            ...
            transactions.stream()
                    ...
                    .forEach(t -> {
                        ...
                        insight.setCategoryName(t.getCategory() != null ? t.getCategory().getName() : "Uncategorized");
                        insight.setDescription(
                                "Transaction of " + t.getAmount() + " VND is significantly above your average");
    ...
    /**
     * Get anomaly transactions (marked as isAnomaly = true)
     */
    @Transactional(readOnly = true)
    @SuppressWarnings("null")
